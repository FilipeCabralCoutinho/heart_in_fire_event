...
def enrollment():
    if request.method == "POST":
        ...
        if not consent_given:
            flash(
                """
                É necessário concordar com os termo de consentimento
                para tratamento dos dados para fins do evento.
                """
            )
            return redirect(url_for("routes.enrollment"))
        ...
        if valid is False:
            ...
            logger.critical(
                f"Fail in validation data, data informed: {valid}"
            )
            return redirect(url_for("routes.enrollment"))

        logger.info(
            f"Information validated for the name {name},"
            " proceeding with registration creation."
        )
...
def update_enrollments(id):
    ...
    if request.method == "POST":
        ...
        logger.info(
            "A update enrollment process has been "
        ...
        valid = validator.validate_all(
            name, cpf, celphone, emergency_contact, email, church
        )

        if valid is False:
            return redirect(
                url_for("routes.update_enrollments", id=enrollment.id)
            )
        elif valid is not None:
            logger.critical(
                f"Fail in validation data, data informed: {valid}"
        ...
        logger.info(
            f"Information validated for the id {enrollment.id},"
            " proceeding with update enrollment."
        )
        ...
        update = service.update_enrollment(
            enrollment, new_enrollment, enrollment_dict
        )
